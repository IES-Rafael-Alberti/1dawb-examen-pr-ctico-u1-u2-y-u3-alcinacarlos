...
def limpiar_consola():
    """
    Limpia la terminal para obtener un resultado claro
    """
    #funciona tanto si el usuario tiene windows o linux
    if os.name == "posix":
        os.system("clear")
    else:
        os.system("cls")
...
def generar_mapa(posicion_jugador:tuple) -> list:
    ...
    #Evitar que en la posición inicial del jugador en el mapa se genere una pista o una trampa.
    if mapa[posicion_jugador[0]][posicion_jugador[1]] != CELDA_VACIA:
...
def pedir_movimiento(mapa: list) -> str:
    ...
    entrada = input("Ingresa tu movimiento (formato: 'u:arriba', 'd:abajo', 'l:izquierda', 'r:derecha', q:salir): ").lower()
...
def simbolo_celda(celda):
    ...
    :param celda: celda a validar
    :return: el símbolo a pintar en la celda
    """
...
def imprimir_mapa_oculto(mapa: list , coords_jugador:tuple):
    """Imprime el estado actual del mapa con el jugador
    :param mapa: lista del mapa actual
    :param coords_jugador: tupla de las coordenadas actuales del jugador
    """
    contador = 1
    print("    " + " ".join([str(i) for i in range(1, DIMENSIONES + 1)]))
    print("   " + "".join(["-" for _ in range(DIMENSIONES*2)]))
    for fila in mapa:
        i = 0
        for celda in fila:
            if i == 0:
                print(str(contador) + " | " + simbolo_celda(celda), end=" ")
                contador += 1
                i += 1
            elif i == 4:
...
def muestra_estado_mapa(mapa, posicion_jugador):
    ...
    coords_jugador = tuple(coord +1 for coord in posicion_jugador)
    imprimir_mapa_oculto(mapa, coords_jugador)
    print(f"Tu posición es {coords_jugador}")
